...
# 자유 입출금 계좌와 저축 계좌는 서로 비슷한 점이 많아서 두 클래스에는 중복되는 코드가 많습니다. 이번 챕터에서 배운 상속을 이용해서 중복되는 코드를 줄여 봅시다.
...
class CheckingAccount(BankAccount):
    ...
    # 여기에 코드를 작성하세요
    def __init__(self, name : str, balance : int, max_spending : int) -> None:
        super().__init__(name,balance)
        self.max_spending = max_spending

    def use_check_card(self, amount : int) -> None:
